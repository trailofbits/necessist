use super::{
    Call, GenericVisitor, LocalFunction, Python, Spanned, Statement, Storage, Test,
    sole_suite_statement, statement_is_instrumentable,
};
use anyhow::Result;
...
impl<'ast> AstVisitor<'ast> for Visitor<'_, '_, '_, 'ast, '_> {
    fn visit_body(&mut self, body: &'ast [Stmt]) {
        if let Some(statement) = sole_suite_statement(body) {
            ...
            self.generic_visitor
                .backend
                .sole_suite_statements
                .insert(statement.span(&self.generic_visitor.source_file));
        }
        for statement in body {
    ...
    fn visit_stmt(&mut self, statement: &'ast Stmt) {
        ...
        drop(storage);
        if statement_is_instrumentable(statement.statement, statement.tokens) {
            self.generic_visitor
                .backend
                .instrumentable_statements
                .insert(statement.span(&self.generic_visitor.source_file));
        }
        let walk = self
            .generic_visitor
